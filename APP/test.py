...
class WonScreen(Screen):
    def on_pre_enter(self):  # Kazanma ekranına girişte verileri kaydet
        self.manager.get_screen('question').pushData()
...
class FailsScreen(Screen):
    ...
    def on_pre_enter(self):  # Kaybetme ekranına girişte verileri kaydet
        self.manager.get_screen('question').pushData()
...
class QuestionScreen(Screen):
    ...
    def pushData(self):
        df = pd.DataFrame([[self.username, self.Score, (self.Score / 5), self.time_left]],
                          columns=["Username", "Score", "Level", "Time"])
        df.to_csv('APP/Db/User-Info.csv', mode='a', sep='|', 
                  header=not pd.io.common.file_exists('APP/Db/User-Info.csv'), index=False)
